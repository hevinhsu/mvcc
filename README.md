...
## Isolation Level
先快速介紹一下為什麼需要 database isolation：這個東西是為了讓資料庫提高吞吐量，在犧牲一定資料正確性下所設計出來的東西。
資料庫為了讓更多人可以同時使用，所以資料庫提供了多個 process/thread 來提供使用者服務，
但在這個過程中，使用者可能會同時操作同一筆資料，這時候就產生了資料不一致，而 isolation level 就是用不同的策略來讓多個 process/thread 同時運行，並讓使用者自行決定要顧及效能還是資料正確性。

一般主流資料庫提供了：Read Uncommitted、Read Committed、Repeatable Read、Snapshot、Serializable 五種 isolation level。

## Why MVCC
上面提到資料庫為了提高吞吐量，會讓多個使用者並行的使用資料，為了避免資料錯誤，我們就會如同 concurrent programming 一樣，
...
## 實作
細節請參閱程式碼。本專案和原文章相同，實作了一個 key,value store，並用簡單的資料結構來呈現概念。更多的細節請參閱[原文](https://notes.eatonphil.com/2024-05-16-mvcc.html)，
以下說明僅說明作者覺得重要的程式碼。
...
### Visibility
在 MVCC 中對同一個 key，儲存了多個版本的 value，不同的 tx 讀取到甚麼資料，主要依靠資料的可見性來運作，根據不同 isolation level，tx 能看到的內容有所不同，實作方式為以下：
...
private boolean isVisible(Transaction tx, Value value) {
	...
	if (isolationLevel == IsolationLevel.ReadCommitted) {
		...
		if (value.getTxStartId() != tx.getId()  // 不是自己 tx 修改的資料
...
實作上原作者會檢查 inprogress 這個 set，少了那段程式碼，仍可以通過測試，因此推測是為了加速使用。用 inprogress set，能避免在檢查 value 時，遍歷整個 value list(不同版本的 value)。
...
### Conflict
延續上面 visibility 我們可以發現 Repeatable Read、Snapshot、Serializable isolation level 在資料可見性上是相同的，他們幾個之間的差別，是當資料在 commit 階段，
會有額外的檢查來保證資正確性，因此這部分會由 conflict 來更進一步區分 Repeatable Read、Snapshot、Serializable isolation level。
...
## 其他
如原文所述，這個時做選擇了簡單版的概念來實作，實際上不同資料庫有不同的實作方式，絕對比現在的版本複雜很多。
而在真實世界中，MVCC 因為保存了多個版本的資料，所以會需要額外進行資料清理，又叫做 ```Vacuuming```。
所以本專案只能歸類在熟悉資料庫 isolation level，並讓人對 MVCC 有初步的瞭解。